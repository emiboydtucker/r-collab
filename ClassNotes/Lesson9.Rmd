...
```
```{r}
#we already did this above but ok
# Let's remove those
coronavirus %>% 
  filter(cases > 0) %>% 
  group_by(date, type) %>%
...
It is useful to know which country is represented by each dot. geom_label is our tool for that.
...
``` {r}
ggplot(data = filter(coronavirus_ttd, confirmed > 10000000)) +
  geom_label(mapping = aes(x = confirmed, y = death, label = country))
```
...
ggplot(data = filter(coronavirus_ttd, confirmed > 10000000)) +
  geom_label(mapping = aes(x = confirmed, y = death, label = country)) +
  xlim(5000000, max(coronavirus_ttd$confirmed))
```
...
#RGEOS WAS FULLY RETIRED IN 2023, MUST US SF LIBRARY: library("rgeos") # install.packages("rgeos")
```
...
See following lesson for more exercises/clarity when trying to use this code and these functions:
https://nt246.github.io/NTRES6940-data-science/lesson9-ggplot-part2.html#combining_dplyr_and_ggplot 
